...
description: Shop Taobao/Tmall with smart search strategies, seller vetting, price negotiation, deal finding guidance, and browser-based product/cart/coupon checks.
metadata:
  clawdbot:
    emoji: "🛍️"
    requires:
...
When the task requires live Taobao/Tmall page inspection, follow the shared **browser-commerce-base** workflow:
- public search/detail pages → `openclaw`
- cart/coupon/order/member pages → `user` only when login state matters
- re-snapshot after SKU changes, coupon expansion, or filter changes
- capture seller badge, final price, and service promises in evidence screenshots

Key browser extraction order on Taobao/Tmall:
- 商品标题
- 当前价 / 到手价
- 店铺类型（旗舰店 / 授权 / 皇冠 / 普通店）
- 销量 / 评价提示
